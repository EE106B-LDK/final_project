...
def rotation_3d(omega, theta):
    ...
    return np.eye(3) + omega_hat * np.sin(omega_norm * theta) + np.matmul(omega_hat, omega_hat) * (1 - np.cos(omega_norm * theta))
...
def hat_3d(xi):
    ...
    v, omega = xi[:3], xi[3:]
    return np.vstack([np.hstack([skew_3d(omega), v.reshape((3, 1))]), np.zeros(4)])
...
def homog_3d(xi, theta):
    ...
    v, omega = xi[:3], xi[3:]
    if not omega.any():
        return np.vstack([np.hstack([np.eye(3), v.reshape((3, 1)) * theta]), np.array([0, 0, 0, 1])])
    R = rotation_3d(omega, theta)
    omega_hat = skew_3d(omega)
    p = (np.matmul(np.eye(3) - R, np.matmul(omega_hat, v)) + omega * np.dot(omega, v) * theta) / (np.linalg.norm(omega)**2)
    return np.vstack([np.hstack([R, p.reshape((3, 1))]), np.array([0, 0, 0, 1])])

def pose_from_homog_3d(g):
    """
    Returns R (3x3) and t (3x) from a 4x4 transformation matrix
    """
    return g[:3, :3], g[:3, 3]


def xi_screw(omega, q, h):
    omega, q = np.array(omega), np.array(q)
    omega_hat = skew_3d(omega)
    v = np.matmul(-omega_hat, q) + h * omega
...
def prod_exp(xi, theta):
    ...
    # YOUR CODE HERE
    g = np.eye(4)
    for x, th in zip(xi.T, theta):
        g = np.matmul(g, homog_3d(x, th))
    return g
